    ...
    for (const op of queue) {
      try {
        if (op.type === 'INSERT_SALE') {
          ...
          await SupabaseClient.delete('sales', `local_id=eq.${op.payload.id}`);
        }
        synced++;
      } catch (err) {
...
const SaleRepository = {
  ...
  /**
   * Exclui uma venda por ID local (tanto no Supabase quanto no localStorage).
   */
  async delete(id) {
    this._deleteFromLocalStorage(id);

    if (!SupabaseClient.isConfigured) {
      console.warn('[SaleRepository] Supabase não configurado. Excluído apenas no localStorage.');
      return id;
    }

    try {
      await SupabaseClient.delete('sales', `local_id=eq.${id}`);
      return id;
    } catch (err) {
      console.warn('[SaleRepository] Offline ao excluir. Enfileirando exclusão:', err.message);
      OfflineQueue.enqueue({ type: 'DELETE_SALE', payload: { id } });
      return id;
    }
  },
  ...
  _deleteFromLocalStorage(id) {
    try {
      ...
      localStorage.setItem('pharmapos_sales', JSON.stringify(filtered));
    } catch (err) {
      console.warn('[SaleRepository] Erro ao excluir do localStorage:', err.message);
    }
  },
